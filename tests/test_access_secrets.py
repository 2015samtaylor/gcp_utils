...
from gcp_utils.access_secrets import access_secret_version
...
def test_access_secret_version_returns_credentials():
    ...
    with patch('gcp_utils.access_secrets.secretmanager.SecretManagerServiceClient', return_value=mock_client):
        mock_client.access_secret_version.return_value = mock_response
        with patch('gcp_utils.access_secrets.Credentials.from_service_account_info') as mock_creds:
...
def test_access_secret_version_logs_error(caplog):
    with patch('gcp_utils.access_secrets.secretmanager.SecretManagerServiceClient', side_effect=Exception('fail')):
        creds = access_secret_version('pid', 'sid', '1')
