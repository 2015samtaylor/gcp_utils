...
from gcp_utils.buckets import send_to_gcs
...
def test_send_to_gcs_uploads_when_not_empty():
    ...
    with patch('gcp_utils.buckets.storage.Client', return_value=mock_client):
...
def test_send_to_gcs_logs_when_empty(caplog):
    ...
    df = pd.DataFrame()
    with patch('gcp_utils.buckets.storage.Client') as mock_client:
        send_to_gcs('test-bucket', 'test/path', df, 'file.csv')
